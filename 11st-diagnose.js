(function () {
  ...
  if (!location.hostname.includes('11st.co.kr')) {
    alert('이 진단 도구는 11번가 장바구니에서만 실행해 주세요.');
    return;
  }
  ...
  function chooseContainer(cb) {
    var node = cb;
    var candidates = [];
    for (var depth = 0; depth < 11 && node; depth += 1, node = node.parentElement) {
      var text = safeText(node);
      var priceCount = (text.match(/[\d,]+\s*원/g) || []).length;
      var controls = node.querySelectorAll ? node.querySelectorAll('input,button,select,[role="button"],[role="spinbutton"]').length : 0;
      if (priceCount >= 1 && controls >= 1 && text.length > 15 && text.length < 7000) {
  ...
  checked.forEach(function (cb) {
    ...
    var controls = Array.prototype.slice.call(
      container.querySelectorAll('input,button,select,[role="button"],[role="spinbutton"]')
    );
    ...
    products.push({
      ...
      quantityControls: controls.filter(looksLikeQuantity).map(attrs),
  ...
  box.style.cssText = 'width:min(940px,96vw);max-height:90vh;background:#fff;border-radius:16px;box-shadow:0 22px 60px rgba(15,23,42,.3);padding:20px;display:flex;flex-direction:column;gap:12px;';

  var title = document.createElement('div');
  title.innerHTML = '<div style="font-size:13px;font-weight:800;color:#dc2626">11번가 수량 진단</div><div style="font-size:22px;font-weight:850;color:#172033;margin-top:4px">진단정보가 준비되었습니다.</div><div style="font-size:13px;color:#667085;margin-top:5px">복사한 내용을 ChatGPT 대화에 그대로 붙여 주세요.</div>';

  var area = document.createElement('textarea');
  area.value = json;
  area.readOnly = true;
  area.style.cssText = 'width:100%;min-height:460px;resize:vertical;border:1px solid #d0d5dd;border-radius:10px;padding:12px;font:12px/1.55 Consolas,monospace;color:#344054;background:#f8fafc;';
  ...
  copy.style.cssText = 'border:0;border-radius:10px;background:#dc2626;color:#fff;font-weight:800;padding:11px 16px;cursor:pointer;';
